...
The default build is LAN-only. Its LAN listener binds to one concrete, non-publicly-routable address on an approved non-tunnel local interface (RFC1918 IPv4, IPv4 link-local, or IPv6 ULA) and refuses to start when none exists. Internet control remains disabled until the developer explicitly deploys a relay in their own Cloudflare account. The Mac accepts only outbound Internet relay connections; no inbound public port is opened.
